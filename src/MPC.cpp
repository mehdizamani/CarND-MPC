...
// It needs to convert the ref_v from mph to meter per second because we have to deal
// with the latency in the main file. 
double ref_v = 55 * 0.44704;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Each term in the objective function has a weight that
	// determines the level of importsance of that term for use
	double w_epsi = 100, w_delta = 200, w_delta_diff = 200 ; 
...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs) {
  ...
  if (ok==true)
  {
	   ...
	   */
	   ...
	   // It does not need to send all of the obtained delta or a values,
	   // because only the first set of these values (from 1 to N) will be impelemented.
	   std::vector<double> output;
	   output.push_back(solution.x[delta_start]);
	   output.push_back(solution.x[a_start]);
	   // but we need the obtained trajectory to show them in the simulator,
	   // therefore we capture obtained x and y and return them back to simulator.
	   for(int i=0; i<N-1; i++)
	   {
